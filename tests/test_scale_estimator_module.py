...
pc1 = sample(shape=shape, num_points=1000, random_sampling=True)
pc1 = scale(pc1, 10) # it is scaled for a better visualizzation


# generate a measurements with noise for the torus point cloud
pc2 = pc1
pc2 = add_white_gaussian_noise(pc1, mu=0, sigma=0.01)       # add noise
...
estimator.configure(iterations=200, dumping=0.7, kernel_threshold=0.01)
...
print("measurements after sim correction", pc_similarity_correction.shape)

# Compute the Error
similarity_error = np.mean(np.linalg.norm(pc1-pc_similarity_correction, axis=1))
print('Mean Square Error (Similarity):', similarity_error)

estimator.configure(iterations=150, dumping=0.7, kernel_threshold=0.01)
...
print("measurements after sim correction", pc_homogeneous_correction.shape)

# Compute the Error
homogeneous_error = np.mean(np.linalg.norm(pc1-pc_homogeneous_correction, axis=1))
