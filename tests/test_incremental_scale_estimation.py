...
def main():
    ...
    for image_key in map_images_keys:
        ...
        points3d_coords_affinity_correction = np.array([transform(Transform, p) for p in points3d_coords_similarity_correction])
        ...
        tvecs_affinity_correction = np.array([transform(Transform, p) for p in tvecs_similarity_correction])
